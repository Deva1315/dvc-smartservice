...
import {
  getAdminGudangProfileRequest,
  type AdminGudangProfileUser,
} from "@/lib/admin-gudang/admin-gudang-profile-client";
...
function mapApiUser(user: AdminGudangProfileUser): DashboardSessionUser {
  return {
    id: user.id,
    name: user.nama,
    email: user.email,
...
export default function AdminGudangProfilePage({ user }: Props) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setIsLoading(true);
        const result = await getAdminGudangProfileRequest();

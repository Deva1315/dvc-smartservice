  ...
  try {
    ...
    if (!tiket) {
      return NextResponse.json(
        { success: false, message: "Tiket servis tidak ditemukan" },
        { status: 404 }
      );
    }
    ...
    console.error("GET DETAIL ADMIN PENJUALAN TIKET ERROR:", error);
    return NextResponse.json(
      { success: false, message: "Gagal mengambil detail tiket servis" },
      { status: 500 }
    );
  }
}